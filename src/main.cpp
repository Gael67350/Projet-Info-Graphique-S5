...
//Initialize OpenGL Version (version 3.0)
	...
	while (isOpened)
	{
		...
		float environmentDiffuse = 1.4f * std::get<1>(lightInfo);
		environment.initLight(std::get<0>(lightInfo), std::get<3>(lightInfo), 0.8f, environmentDiffuse <= 1.f ? environmentDiffuse : 1.f);
